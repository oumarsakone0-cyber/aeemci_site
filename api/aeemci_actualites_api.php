...
function publicRow(array $row, bool $compact = false): array {
    ...
    $date = $row['date_specifique'] ?: ($row['date_debut'] ?: $row['created_at']);
    $payload = [
        'id' => (int)$row['id'],
        'titre' => $row['titre'],
        'title' => $row['titre'],
        ...
        'date_specifique' => $row['date_specifique'],
        'date' => $date,
        'auteur' => $row['auteur'],
        'author' => $row['auteur'],
...
try {
    ...
    if ($action === 'latest' || $action === 'list') {
        ...
        if (!empty($_GET['search'])) {
            $where[] = '(titre LIKE :q OR texte_affichage LIKE :q OR texte_detaille LIKE :q)';
            $params[':q'] = '%' . $_GET['search'] . '%';
        }
        $sqlWhere = $where ? 'WHERE ' . implode(' AND ', $where) : '';
        $stmt = db()->prepare("SELECT * FROM aeemci_actualites {$sqlWhere} ORDER BY COALESCE(date_specifique, date_debut, DATE(created_at)) DESC, id DESC LIMIT {$limit} OFFSET {$offset}");
        $stmt->execute($params);
