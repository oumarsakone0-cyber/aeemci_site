...
define('PUBLIC_PAGE_URL', getenv('SEMINARISTES_CCR_URL') ?: 'https://www.aeemci-ce.ci/seminaristes_ccr');
define('API_URL', getenv('SEMINARISTES_CCR_API_URL') ?: 'https://api.aeemci-ce.ci/senafoi/seminaristes_ccr.php');
define('CCR_TABLE', 'aeemci_seminaristes_ccr_2026');
define('CCR_OLD_TABLE', 'aeemci_seminaristes_ccr');
define('CCR_QUOTA', 2);
...
function ensureSchema(): void {
    db()->exec("CREATE TABLE IF NOT EXISTS aeemci_seminaristes_ccr_2026 (
      ...
      PRIMARY KEY (id),
      UNIQUE KEY uq_sem_ccr_matricule (matricule),
      KEY idx_sem_ccr_secretariat (secretariat),
      KEY idx_sem_ccr_payment (statut_paiement, transaction_id)
    ...
    $stmt = db()->prepare("
        ...
        WHERE TABLE_SCHEMA = DATABASE()
          AND TABLE_NAME = 'aeemci_seminaristes_ccr_2026'
          AND COLUMN_NAME = 'ref_paiement'
    ");
    $stmt->execute();
    if ((int)$stmt->fetchColumn() === 0) {
        db()->exec("ALTER TABLE aeemci_seminaristes_ccr_2026 ADD COLUMN ref_paiement VARCHAR(160) DEFAULT NULL AFTER transaction_id");
    }
}
...
function migrateCcrPaidAbidjanFromOld(): void {
    if (!tableExists(CCR_OLD_TABLE)) return;
    db()->exec("INSERT IGNORE INTO aeemci_seminaristes_ccr_2026
      ...
      FROM aeemci_seminaristes_ccr old_ccr
      WHERE old_ccr.depart_abidjan = 1
        AND old_ccr.statut_paiement = 'PAYE'
        AND NOT EXISTS (SELECT 1 FROM aeemci_seminaristes_ccr_2026 new_ccr WHERE new_ccr.matricule = old_ccr.matricule)");
}
...
function ensureJournalMatricules(): void {
    ...
    db()->exec("INSERT IGNORE INTO aeemci_matricules_emis (matricule, famille)
                SELECT matricule, 'CCR' FROM aeemci_seminaristes_ccr_2026
                WHERE matricule LIKE 'SEM26CCR%'");
}
...
function nextMatricule(): string {
    ...
    $vivants = (int)db()->query(
        "SELECT COALESCE(MAX(CAST(SUBSTRING(matricule, 9) AS UNSIGNED)), 0)
         FROM aeemci_seminaristes_ccr_2026 WHERE matricule LIKE 'SEM26CCR%'"
    )->fetchColumn();

    $emis = (int)db()->query(
        "SELECT COALESCE(MAX(CAST(SUBSTRING(matricule, 9) AS UNSIGNED)), 0)
         FROM aeemci_matricules_emis WHERE famille = 'CCR' AND matricule LIKE 'SEM26CCR%'"
    )->fetchColumn();

    $next = max($vivants, $emis) + 1;
    $matricule = 'SEM26CCR' . str_pad((string)$next, 3, '0', STR_PAD_LEFT);

    db()->prepare("INSERT IGNORE INTO aeemci_matricules_emis (matricule, famille) VALUES (?, 'CCR')")
        ->execute([$matricule]);
...
function listRows(): void {
    ...
    $stmt = db()->prepare("SELECT * FROM aeemci_seminaristes_ccr_2026 {$where} ORDER BY secretariat ASC, id DESC");
    $stmt->execute($params);
    respond(['success' => true, 'data' => $stmt->fetchAll()]);
}

function stats(): void {
    $rows = db()->query("SELECT secretariat, COUNT(*) AS total, SUM(depart_abidjan = 1) AS depart_abidjan, COUNT(*) AS valides FROM aeemci_seminaristes_ccr_2026 WHERE statut_paiement IN ('PAYE', 'GRATUIT') GROUP BY secretariat ORDER BY secretariat ASC")->fetchAll();
    $total = db()->query("SELECT COUNT(*) FROM aeemci_seminaristes_ccr_2026 WHERE statut_paiement IN ('PAYE', 'GRATUIT')")->fetchColumn();
...
function createRow(): void {
    ...
    $count = db()->prepare("SELECT COUNT(*) FROM aeemci_seminaristes_ccr_2026 WHERE secretariat = ? AND statut_paiement IN ('PAYE', 'GRATUIT')");
    $count->execute([$secretariat]);
    if ((int)$count->fetchColumn() >= CCR_QUOTA) {
        respond(['success' => false, 'message' => 'Quota atteint: 2 membres maximum pour ce secretariat.'], 409);
    }
    ...
    $stmt = db()->prepare("INSERT INTO aeemci_seminaristes_ccr_2026
    ...
    if (!empty($wave['wave_launch_url'])) {
        ...
        $up = db()->prepare("UPDATE aeemci_seminaristes_ccr_2026 SET transaction_id = ?, ref_paiement = ?, updated_at = NOW() WHERE matricule = ?");
    ...
    db()->prepare("DELETE FROM aeemci_seminaristes_ccr_2026 WHERE matricule = ? AND statut_paiement = 'EN_ATTENTE'")->execute([$matricule]);
...
function getByMatricule(string $matricule): array {
    $stmt = db()->prepare("SELECT * FROM aeemci_seminaristes_ccr_2026 WHERE matricule = ?");
...
function paymentReturn(): void {
    ...
    if ($matricule !== '') {
        ...
        db()->prepare("UPDATE aeemci_seminaristes_ccr_2026 SET statut_paiement = ?, wave_status = ?, payment_status_wave = ?, ref_paiement = COALESCE(ref_paiement, transaction_id), updated_at = NOW() WHERE matricule = ?")
...
function paymentCallback(): void {
    ...
    if ($matricule !== '') {
        ...
        db()->prepare("UPDATE aeemci_seminaristes_ccr_2026 SET statut_paiement = ?, wave_status = ?, payment_status_wave = ?, transaction_id = COALESCE(NULLIF(?, ''), transaction_id), ref_paiement = COALESCE(NULLIF(?, ''), ref_paiement, transaction_id), updated_at = NOW() WHERE matricule = ?")
...
function updateRow(): void {
    ...
    $lire = db()->prepare("SELECT * FROM aeemci_seminaristes_ccr_2026 WHERE id = ? LIMIT 1");
    $lire->execute([$id]);
    $actuel = $lire->fetch();
    ...
    if ($secretariat !== $actuel['secretariat']) {
        $count = db()->prepare(
            "SELECT COUNT(*) FROM aeemci_seminaristes_ccr_2026
             WHERE secretariat = ? AND statut_paiement IN ('PAYE', 'GRATUIT') AND id <> ?"
        );
        $count->execute([$secretariat, $id]);
        if ((int)$count->fetchColumn() >= CCR_QUOTA) {
            respond(['success' => false, 'message' => "Quota atteint pour {$secretariat} : 2 membres maximum."], 409);
        }
    }
    ...
    $stmt = db()->prepare("UPDATE aeemci_seminaristes_ccr_2026
...
function deleteRow(): void {
    ...
    $secretariat = validateSecretariat($data['secretariat'] ?? '');
    if ($id <= 0) respond(['success' => false, 'message' => 'ID requis.'], 422);
    $stmt = db()->prepare("DELETE FROM aeemci_seminaristes_ccr_2026 WHERE id = ? AND secretariat = ?");
