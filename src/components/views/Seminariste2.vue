      ...
      <div class="form-wrapper">
        ...
        <div v-if="currentStep === 0 && !showResultsTable && !showParticipationChoice && !showProfileView" class="step-content">
          <div class="welcome-card">
            ...
            <div class="quota-panel" :class="{ 'quota-panel-full': quotaInfo.is_full }">
              <div class="quota-copy">
                <span class="quota-label">{{ quotaStatusText }}</span>
                <strong>Quota : {{ quotaTotal.toLocaleString('fr-FR') }} places</strong>
                <small>{{ quotaInfo.inscriptions_count.toLocaleString('fr-FR') }} inscription{{ quotaInfo.inscriptions_count > 1 ? 's' : '' }} valide{{ quotaInfo.inscriptions_count > 1 ? 's' : '' }} pour 2026</small>
              </div>
              <div class="quota-available">
                <span>{{ availablePlaces.toLocaleString('fr-FR') }}</span>
                <small>place{{ availablePlaces > 1 ? 's' : '' }} disponible{{ availablePlaces > 1 ? 's' : '' }}</small>
              </div>
            </div>
        ...
        <div v-if="showFormContent" class="quota-strip" :class="{ 'quota-strip-full': quotaInfo.is_full }">
          ...
          <small>Quota {{ quotaTotal.toLocaleString('fr-FR') }} - {{ quotaInfo.inscriptions_count.toLocaleString('fr-FR') }} inscription{{ quotaInfo.inscriptions_count > 1 ? 's' : '' }} valide{{ quotaInfo.inscriptions_count > 1 ? 's' : '' }} 2026</small>
        </div>
...
const canOpenNewInscription = computed(() => stopInscriptions.value && (!quotaInfo.value.is_full || hasStartedInscription.value))
