  ...
  <div class="min-h-screen bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 relative overflow-hidden">
    ...
    <div class="relative z-10 p-4 lg:p-6" style="max-width: 1600px; margin: 0 auto;">
        ...
        <div class="flex items-center space-x-3 lg:space-x-4">
          ...
          <div>
            <h1 class="text-xl lg:text-3xl font-bold text-gray-800 flex items-center">
              <svg class="w-6 h-6 lg:w-8 lg:h-8 text-indigo-600 mr-2 lg:mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"/>
              </svg>
              COMMISSIONS CIMA {{ new Date().getFullYear() }}
            </h1>
            <p class="text-gray-600 text-xs lg:text-base">Gestion des inscriptions aux commissions</p>
          </div>
        </div>
      ...
      <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-6 mb-6 lg:mb-8">
        <div class="bg-white rounded-xl shadow-lg border-l-4 border-indigo-500 overflow-hidden transform hover:scale-105 transition-transform">
          <div class="p-4 lg:p-6">
            <div class="flex items-center justify-between" style="padding: 10px;">
              ...
              <div class="p-2 lg:p-3 bg-indigo-100 rounded-full">
        ...
        <div class="bg-white rounded-xl shadow-lg border-l-4 border-blue-500 overflow-hidden transform hover:scale-105 transition-transform">
          <div class="p-4 lg:p-6">
            <div class="flex items-center justify-between" style="padding: 10px;">
              <div>
                <p class="text-xs lg:text-sm font-medium text-gray-600">Hommes</p>
                <p class="text-xl lg:text-2xl font-bold text-blue-600">{{ stats.male_count }}</p>
              </div>
        ...
        <div class="bg-white rounded-xl shadow-lg border-l-4 border-pink-500 overflow-hidden transform hover:scale-105 transition-transform">
          <div class="p-4 lg:p-6">
            <div class="flex items-center justify-between" style="padding: 10px;">
              <div>
                <p class="text-xs lg:text-sm font-medium text-gray-600">Femmes</p>
                <p class="text-xl lg:text-2xl font-bold text-pink-600">{{ stats.female_count }}</p>
              </div>
              <div class="p-2 lg:p-3 bg-pink-100 rounded-full">
                <svg class="w-5 h-5 lg:w-6 lg:h-6 text-pink-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/>
                </svg>
              </div>
            </div>
        ...
        <div class="bg-white rounded-xl shadow-lg border-l-4 border-purple-500 overflow-hidden transform hover:scale-105 transition-transform">
          <div class="p-4 lg:p-6">
            <div class="flex items-center justify-between" style="padding: 10px;">
              <div>
                <p class="text-xs lg:text-sm font-medium text-gray-600">Commissions</p>
                <p class="text-xl lg:text-2xl font-bold text-purple-600">{{ stats.par_commission?.length || 0 }}</p>
              </div>
      ...
      <div class="bg-white rounded-xl shadow-lg border border-gray-200 mb-6 lg:mb-8" style="margin-top: 20px;">
        <div class="px-6 py-4 bg-gradient-to-r from-indigo-100 to-purple-100 border-b" style="padding: 8px">
        ...
        <div class="p-6">
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div v-for="comm in stats.par_commission" :key="comm.commission" class="bg-gradient-to-br from-indigo-50 to-purple-50 rounded-lg p-4 border border-indigo-200" style="padding: 10px;">
              <div class="flex items-start justify-between mb-3">
                <div class="flex-1">
                  <h4 class="font-semibold text-gray-800 text-sm">{{ comm.commission }}</h4>
                  <p class="text-2xl font-bold text-indigo-600 mt-1">{{ comm.total }}</p>
                </div>
                <div class="text-right">
                  <div class="text-xs text-gray-600">
                    <span class="text-blue-600 font-semibold">♂ {{ comm.male }}</span>
                    <span class="mx-1">|</span>
                    <span class="text-pink-600 font-semibold">♀ {{ comm.female }}</span>
                  </div>
                </div>
              </div>
              <div class="w-full bg-gray-200 rounded-full h-2 overflow-hidden">
                <div class="bg-gradient-to-r from-indigo-500 to-purple-500 h-2 rounded-full transition-all duration-300" :style="{ width: ((comm.total / stats.total_inscriptions) * 100) + '%' }"></div>
              </div>
              <p class="text-xs text-gray-500 mt-2">{{ ((comm.total / stats.total_inscriptions) * 100).toFixed(1) }}% du total</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white rounded-xl shadow-lg border border-gray-200 mb-6 lg:mb-8" style="margin-top: 20px;">
        <div class="p-4 lg:p-6">
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-4" style="padding: 10px;">
            <div>
            ...
            <div>
              ...
              >
                ...
                <option v-for="comm in stats.par_commission" :key="comm.commission" :value="comm.commission">{{ comm.commission }}</option>
              </select>
            </div>
            ...
            <div>
              ...
              <select v-model="pagination.per_page" @change="changePerPage" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent text-sm">
          ...
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gradient-to-r from-indigo-50 to-purple-50">
              <tr>
                ...
                <th class="px-3 lg:px-6 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Commission</th>
                <th class="px-3 lg:px-6 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Contact</th>
                <th class="px-3 lg:px-6 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Âge</th>
                <th class="px-3 lg:px-6 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Statut</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              ...
              <tr v-else v-for="registration in registrations" :key="registration.id" class="hover:bg-indigo-50 transition-colors duration-150">
                ...
                <td class="px-3 lg:px-6 py-4 whitespace-nowrap">
                  <img 
                    v-if="registration.photo_url" 
                    :src="registration.photo_url" 
                    :alt="`${registration.prenom} ${registration.nom}`"
                    class="h-10 w-10 lg:h-12 lg:w-12 rounded-full object-cover border-2 border-indigo-200"
                    @error="handleImageError"
                  >
                  <div v-else class="h-10 w-10 lg:h-12 lg:w-12 rounded-full bg-gradient-to-br from-indigo-400 to-purple-500 flex items-center justify-center text-white font-bold text-sm lg:text-base">
                ...
                <td class="px-3 lg:px-6 py-4 whitespace-nowrap">
                  <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-indigo-100 text-indigo-800 font-mono border border-indigo-200">
                    {{ registration.matricule }}
                  </span>
                  <br/>
                  <span v-if="registration.niveau_etude" class="text-gray-600 text-xs mt-1">
                    {{ registration.niveau_etude }}
                  </span>
                </td>
                ...
                <td class="px-3 lg:px-6 py-4 whitespace-nowrap">
                  <div class="text-xs lg:text-sm text-gray-900">{{ registration.contact }}</div>
                  <div v-if="registration.contact_parent" class="text-xs text-gray-500 mt-1">
                    Parent: {{ registration.contact_parent }}
                  </div>
                </td>
                
                <td class="px-3 lg:px-6 py-4 whitespace-nowrap">
                  <span v-if="registration.age" class="inline-flex items-center px-2 py-1 rounded-md text-xs font-medium bg-gray-100 text-gray-800">
                ...
                <td class="px-3 lg:px-6 py-4 whitespace-nowrap">
                  <span v-if="registration.statut === 'actif'" class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-teal-100 text-teal-800 border border-teal-200">
                    <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                    </svg>
                    Actif
                  </span>
                  <span v-else class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-800 border border-gray-200">
                    Inactif
                  </span>
                </td>
                ...
                <td class="px-3 lg:px-6 py-4 whitespace-nowrap text-sm font-medium">
                  <div class="flex items-center space-x-1 lg:space-x-2">
                    <button @click="viewRegistration(registration)" class="text-indigo-600 hover:text-indigo-900 p-1.5 rounded-lg hover:bg-indigo-50 transition-colors" title="Voir détails">
                      <svg class="w-4 h-4 lg:w-5 lg:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <button v-if="registration.photo_url" @click="viewPhoto(registration)" class="text-blue-600 hover:text-blue-900 p-1.5 rounded-lg hover:bg-blue-50 transition-colors" title="Voir photo">
                      <svg class="w-4 h-4 lg:w-5 lg:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div v-if="showViewModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" @click.self="closeViewModal">
      <div class="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto">
        <div class="px-6 py-4 border-b border-gray-200 flex items-center justify-between bg-gradient-to-r from-indigo-50 to-purple-50" style="padding: 8px;">
        ...
        <div v-if="selectedRegistration" class="p-6 space-y-6" style="padding: 8px;">
          <div class="flex items-start space-x-4 pb-6 border-b">
            <img 
              v-if="selectedRegistration.photo_url" 
              :src="selectedRegistration.photo_url" 
              :alt="`${selectedRegistration.prenom} ${selectedRegistration.nom}`"
              class="h-24 w-24 rounded-full object-cover border-4 border-indigo-200"
            >
            <div v-else class="h-24 w-24 rounded-full bg-gradient-to-br from-indigo-400 to-purple-500 flex items-center justify-center text-white font-bold text-3xl">
            ...
            <div class="flex-1">
              ...
              <div class="mt-3 space-y-2">
                <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-indigo-100 text-indigo-800 font-mono">
                  {{ selectedRegistration.matricule }}
                </span>
                <br/>
                <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-purple-100 text-purple-800">
                  {{ selectedRegistration.commission }}
                </span>
              </div>
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="space-y-4">
              <h5 class="font-semibold text-gray-700 border-b pb-2">Informations Personnelles</h5>
              <div>
                <label class="block text-sm font-medium text-gray-500">Date de naissance</label>
                <p class="text-gray-900 mt-1">{{ formatDate(selectedRegistration.date_naissance) }}</p>
              </div>
              <div v-if="selectedRegistration.age">
                <label class="block text-sm font-medium text-gray-500">Âge</label>
                <p class="text-gray-900 mt-1">{{ selectedRegistration.age }} ans</p>
              </div>
              <div v-if="selectedRegistration.niveau_etude">
                <label class="block text-sm font-medium text-gray-500">Niveau d'étude</label>
                <p class="text-gray-900 mt-1">{{ selectedRegistration.niveau_etude }}</p>
              </div>
            </div>

            <div class="space-y-4">
              <h5 class="font-semibold text-gray-700 border-b pb-2">Contact</h5>
              <div>
                <label class="block text-sm font-medium text-gray-500">Téléphone</label>
                <p class="text-gray-900 mt-1">{{ selectedRegistration.contact }}</p>
              </div>
              <div v-if="selectedRegistration.contact_parent">
                <label class="block text-sm font-medium text-gray-500">Contact Parent</label>
                <p class="text-gray-900 mt-1">{{ selectedRegistration.contact_parent }}</p>
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-500">Statut</label>
                <span :class="selectedRegistration.statut === 'actif' ? 'bg-teal-100 text-teal-800' : 'bg-gray-100 text-gray-800'" class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium mt-1">
                  {{ selectedRegistration.statut === 'actif' ? 'Actif' : 'Inactif' }}
                </span>
              </div>
            </div>
...
const commissionsDisponibles = ref([])
...
const loadData = async () => {
  loading.value = true
  try {
    await Promise.all([loadRegistrations(), loadStats(), loadCommissions()])
  } catch (error) {
...
const loadRegistrations = async () => {
  try {
    ...
    let url = `${API_BASE_URL}/commission_cima.php?action=list_commission_registrations`
    ...
    if (filters.value.commission) {
      url += `&commission=${encodeURIComponent(filters.value.commission)}`
    }
    ...
    if (filters.value.niveau_etude) {
      url += `&niveau_etude=${encodeURIComponent(filters.value.niveau_etude)}`
    }
...
const loadStats = async () => {
  try {
    const response = await fetch(`${API_BASE_URL}/commission_cima.php?action=stats_commission`)
    const data = await response.json()

    if (!data.success) return

    stats.value = {
      total_inscriptions: parseInt(data.total_inscriptions) || 0,
      male_count: parseInt(data.male_count) || 0,
      female_count: parseInt(data.female_count) || 0,
      par_commission: data.par_commission || [],
      par_niveau_etude: data.par_niveau_etude || [],
      par_tranche_age: data.par_tranche_age || [],
...
const loadCommissions = async () => {
  try {
    const response = await fetch(`${API_BASE_URL}/commission_cima.php?action=get_commissions_list`)
    const data = await response.json()
    
    if (data.success) {
      commissionsDisponibles.value = data.commissions || []
    }
  } catch (error) {
    console.error('Erreur loadCommissions:', error)
  }
}
...
const viewPhoto = (registration) => {
  selectedPhoto.value = registration.photo_url
...
const exportExcel = () => {
  ...
  if (filters.value.commission) params.append('commission', filters.value.commission)
  ...
  window.open(`${API_BASE_URL}/commission_cima.php?action=export_excel&${params.toString()}`, '_blank')
}
...
const exportPDF = () => {
  ...
  window.open(`${API_BASE_URL}/commission_cima.php?action=export_pdf&${params.toString()}`, '_blank')
}
...
const generateSingleBadge = async (doc, registration, x, y, width, height) => {
  ...
  // Fond du badge commission
  try {
    const background = new Image()
    background.crossOrigin = 'anonymous'
    background.src = 'https://res.cloudinary.com/r-sidence-meubl-e/image/upload/v1767088494/ixoggbpo2ppyb7kvwomz.jpg'
    
    await new Promise((res, rej) => {
      background.onload = res
      background.onerror = rej
    })
    doc.addImage(background, 'JPG', x, y, width, height)
  } catch (e) {
  ...
  // QR Code avec le matricule
  try {
    const qrUrl = `${registration.matricule}`
...
const downloadSelectedBadges = async () => {
  ...
  try {
    ...
    const filename = `badges_selectionnes_commission_${new Date().toISOString().split('T')[0]}.pdf`
    doc.save(filename)
  } catch (error) {
...
const downloadAllBadges = async () => {
  try {
    ...
    const filename = `tous_les_badges_commission_${new Date().toISOString().split('T')[0]}.pdf`
    doc.save(filename)
  } catch (error) {
