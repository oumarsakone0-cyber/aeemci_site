  ...
  <div class="sr-page">
    <header class="sr-hero">
      <div>
        ...
        <h1>Inscription des membres de secrétariat</h1>
        <p>Chaque secrétariat peut enregistrer jusqu'à 10 membres. Les inscriptions sont gratuites, sauf départ depuis Abidjan.</p>
      </div>
      <div class="hero-card">
        <strong>{{ selectedSecretariat || 'Sélectionnez un secrétariat' }}</strong>
        <span>{{ members.length }}/10 membres enregistrés</span>
      </div>
    </header>
      ...
      <section class="panel selector-panel">
        ...
        <form v-if="selectedSecretariat" class="member-form" @submit.prevent="submitMember">
          <div class="form-head">
            ...
            <span :class="{ full: quotaFull }">{{ members.length }}/10</span>
          </div>
          ...
          <div class="transport-box">
            ...
            <small v-if="form.depart_abidjan" class="payment-note">Le membre sera enregistré dans la liste uniquement après confirmation du paiement Wave.</small>
          </div>
      ...
      <section class="panel list-panel">
        <div class="list-head">
          <div>
            ...
            <p v-if="selectedSecretariat">{{ selectedSecretariat }} · quota 10 membres</p>
...
const API_URL = 'https://api.aeemci-ce.ci/senafoi/seminaristes_sr.php'
...
const quotaFull = computed(() => members.value.length >= 10)
const initials = computed(() => `${form.prenom?.[0] || ''}${form.nom?.[0] || ''}`.toUpperCase() || 'SR')
...
async function loadMembers() {
  ...
  localStorage.setItem('seminaristes_sr_secretariat', selectedSecretariat.value)
  loading.value = true
  error.value = ''
  try {
...
async function deleteMember(member) {
  ...
  try {
    ...
    success.value = 'Membre supprimé. Vous pouvez le remplacer.'
    await loadMembers()
  } catch (e) {
    error.value = e.message
  }
...
onMounted(() => {
  ...
  selectedSecretariat.value = params.get('secretariat') || localStorage.getItem('seminaristes_sr_secretariat') || ''
  const payment = params.get('payment')
  const matricule = params.get('matricule')
  if (payment === 'success') paymentMessage.value = `Paiement confirmé pour ${matricule}.`
  if (payment === 'pending') error.value = `Paiement non confirmé pour ${matricule}. Le membre ne sera affiché qu'après confirmation Wave.`
