          ...
          <div class="logo">
            <div class="logo-icon">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
                <path d="M12 1L3 5v6c0 5.55 3.84 10.74 9 12 5.16-1.26 9-6.45 9-12V5l-9-4zm0 5.5l1.65 3.35 3.7.54-2.68 2.6.63 3.69L12 15.94l-3.3 1.74.63-3.69-2.68-2.6 3.7-.54L12 6.5z"/>
              </svg>
            </div>
            <div class="logo-text">
              <h1>AEEMCI</h1>
              <p>Imarat du Séminaire National de Formation Islamique</p>
            </div>
          </div>
        </div>
          ...
          <div class="welcome-card">
            ...
            <div class="quota-panel">
              <div class="quota-copy">
                <span class="quota-label">Imarat SENAFOI</span>
                <strong>{{ stats.total.toLocaleString('fr-FR') }} membre{{ stats.total > 1 ? 's' : '' }} inscrit{{ stats.total > 1 ? 's' : '' }}</strong>
                <small>{{ fonctionsPourvues }} fonction{{ fonctionsPourvues > 1 ? 's' : '' }} pourvue{{ fonctionsPourvues > 1 ? 's' : '' }} sur {{ fonctions.length }}</small>
              </div>
            ...
            <div class="choice-section">
              ...
              <p class="choice-subtitle">Inscrivez un membre de l'Imarat ou consultez la liste déjà enregistrée.</p>
              ...
              <div class="choice-buttons">
                <button v-if="formVisible" type="button" class="choice-btn choice-btn-primary" @click="currentView = 'form'">
                  ...
                  <div class="choice-content">
                    <span class="choice-title">Inscrire un membre de l'Imarat</span>
                ...
                <div v-else class="closed-card">
                  ...
                  <strong>Les inscriptions de l'Imarat sont clôturées.</strong>
                ...
                <button type="button" class="choice-btn choice-btn-secondary" @click="showList">
                  ...
                  <div class="choice-content">
                    ...
                    <span class="choice-desc">Filtrer par fonction</span>
                  </div>
                  <svg class="choice-arrow" width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M8.59 16.59L10 18l6-6-6-6-1.41 1.41L13.17 12z"/>
                  </svg>
        ...
        <section v-else-if="formVisible && currentView === 'form'" class="step-content">
          <div class="step-card">
            ...
            <form class="registration-form" @submit.prevent="submit">
              <div class="fonction-choice">
                <button
                  v-for="f in fonctions"
                  :key="f.value"
                  type="button"
                  class="fn-btn"
                  :class="{ active: form.fonction === f.value }"
                  @click="form.fonction = f.value"
                >
                  <span class="fn-rank">{{ f.rang }}</span>
                  <span class="fn-text">
              ...
              <div class="form-grid">
                ...
                <div class="form-group">
                  ...
                  <select v-model="form.fonction" required>
                    <option value="">Choisir une fonction</option>
                    <option v-for="f in fonctions" :key="f.value" :value="f.value">{{ f.label }}</option>
                  </select>
                </div>
              </div>
              ...
              <div class="step-nav">
                ...
                <button type="submit" class="btn btn-primary" :disabled="saving || uploading || !form.fonction">
        ...
        <section v-else class="step-content">
          <div class="results-card">
            ...
            <div class="summary-row">
              <article v-for="f in fonctions" :key="f.value">
                <span>{{ f.label }}</span>
                <strong>{{ compteFonction(f.value) }}</strong>
              </article>
            </div>

            <div class="filters">
              <select v-model="selectedFonction" @change="loadRows">
            ...
            <div v-else class="members-list">
              <article v-for="row in filteredRows" :key="row.id" class="member-card">
                ...
                <div class="member-main">
                  ...
                  <span>{{ labelFonction(row.fonction) }} · {{ row.contact }}</span>
...
const API_URL = 'https://api.aeemci-ce.ci/senafoi/imarat_senafoi.php'
const UPLOAD_URL = 'https://api.aeemci-ce.ci/cloud/upload.php'
const CLOUD_API_KEY = 'SAKONE25_04_2026_medias'

// La valeur envoyée à l'API est la forme canonique stockée en base.
const fonctions = [
  { value: 'AMIR', label: 'Amir', rang: 'A', desc: "Dirige l'Imarat du séminaire" },
  { value: '1ER VICE AMIR', label: '1er Vice-Amir', rang: 'V1', desc: "Premier adjoint de l'Amir" },
  { value: '2EME VICE AMIR', label: '2ème Vice-Amir', rang: 'V2', desc: "Second adjoint de l'Amir" },
  { value: 'VICE AMIRATE', label: 'Vice-Amirate', rang: 'VA', desc: 'Responsable des sœurs séminaristes' },
]

const currentView = ref('home')
const rows = ref([])
const stats = ref({ total: 0, avec_photo: 0, par_fonction: {} })
const selectedFonction = ref('')
const search = ref('')
...
const fonctionsPourvues = computed(
  () => fonctions.filter((f) => compteFonction(f.value) > 0).length
)

const filteredRows = computed(() => {
  const needle = search.value.trim().toLowerCase()
  if (!needle) return rows.value
  return rows.value.filter((row) => [row.nom, row.prenom, row.matricule, row.contact, row.fonction]
    .some((value) => String(value || '').toLowerCase().includes(needle)))
})

function defaultForm() {
  return { fonction: '', nom: '', prenom: '', contact: '', photo: '', photo_id: '' }
}

function labelFonction(value) {
  return fonctions.find((f) => f.value === value)?.label || value || '—'
}

function compteFonction(value) {
  return Number(stats.value.par_fonction?.[value] || 0)
}
...
async function loadRows() {
  ...
  error.value = ''
  try {
    const qs = selectedFonction.value ? `&fonction=${encodeURIComponent(selectedFonction.value)}` : ''
...
async function submit() {
  ...
  if (!form.fonction) {
    error.value = "Choisissez la fonction au sein de l'Imarat."
    return
  }
  saving.value = true
  error.value = ''
...
async function downloadReceipt(row) {
  ...
  const libelle = labelFonction(row.fonction).toUpperCase()
  const date = new Date(String(row.created_at || Date.now()).replace(' ', 'T')).toLocaleDateString('fr-FR')
  const qrData = await QRCode.toDataURL(`SENAFOI 2026 | IMARAT | ${libelle} | ${row.matricule} | ${row.prenom} ${row.nom}`)
  ...
  doc.setFontSize(11)
  doc.text(`IMARAT · SENAFOI 2026`, pageWidth - 18, 39, { align: 'right' })
  ...
  const lignes = [
    ['Fonction', labelFonction(row.fonction)],
    ['Contact', row.contact || '-'],
    ['Montant paye', '0 FCFA'],
  ...
  doc.setFont('helvetica', 'normal')
  doc.text(
    `Ce recu confirme une inscription gratuite au sein de l'Imarat du SENAFOI 2026.`,
    46, 235, { maxWidth: 130 }
  )
  ...
  doc.save(`recu_imarat_${safeName(row.matricule)}.pdf`)
}
...
.fonction-choice { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 12px; }
.fn-btn { display: grid; grid-template-columns: 44px 1fr; gap: 12px; align-items: center; text-align: left; border: 2px solid var(--line); border-radius: 0; padding: 14px; background: #fff; cursor: pointer; transition: .2s ease; font: inherit; }
.fn-btn:hover { border-color: var(--green-mist); transform: translateY(-1px); }
.fn-btn.active { border-color: var(--green); background: linear-gradient(135deg, var(--green-mist), #fff); box-shadow: var(--shadow); }
.fn-rank { width: 44px; height: 44px; border-radius: 0; display: grid; place-items: center; background: var(--green-mist); color: var(--green); font-weight: 950; font-size: 15px; }
.fn-btn.active .fn-rank { background: var(--green); color: #fff; }
.fn-text strong { display: block; font-size: 15.5px; font-weight: 950; color: var(--ink); }
.fn-text small { display: block; margin-top: 3px; font-size: 12px; color: var(--muted); line-height: 1.35; }
...
.summary-row article { border: 1px solid var(--line); border-radius: 0; padding: 14px; background: linear-gradient(135deg, var(--surface), #fff); }
.summary-row span { font-size: 11.5px; color: var(--muted); font-weight: 900; }
.summary-row strong { display: block; font-size: 28px; color: var(--ink); }
...
@media (max-width: 780px) {
  ...
  .choice-buttons, .form-grid, .filters, .fonction-choice { grid-template-columns: 1fr; }
